...
class GeoJSONStyledFeatures {
    ...
    _updateEntities(newStyledFeatures) {
    ...
    _updatePolygonPrimitive(newStyledFeatures) {
        const polygonPrimitives = newStyledFeatures.filter(({ primitive }) => primitive.type === 'polygon' && !primitive.clampToGround);
        const noActions = polygonPrimitives.length ? polygonPrimitives.every(({ action }) => action === 'none') : false;
        if (noActions) {
            return;
        }
    ...
    _updateGroundPolygonPrimitive(newStyledFeatures) {
        const groundPolygonPrimitives = newStyledFeatures.filter(({ primitive }) => primitive.type === 'polygon' && !!primitive.clampToGround);
        const noActions =  groundPolygonPrimitives.length ? groundPolygonPrimitives.every(({ action }) => action === 'none') : false;
        if (noActions) {
            return;
        }
        ...
        this._timeout = setTimeout(() => {
            ...
            if (this._styleFunction) {
                this._styleFunction({
                    ...
                    .then((styledFeatures) => {
                        this._updateEntities(styledFeatures);
                        this._updatePolygonPrimitive(styledFeatures);
                        this._updateGroundPolygonPrimitive(styledFeatures);

                        this._styledFeatures = [...styledFeatures];
                        onUpdate(this._styledFeatures);
    ...
    setStyleFunction(styleFunction, onUpdate) {
