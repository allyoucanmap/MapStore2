...
const CardListColumns = ({
    ...
    return (
        <div className="ms-card-list-columns">
            <div className="ms-card-list-columns-placeholder"></div>
            <div
                className="ms-card-list-columns-container"
                ref={container}
                onPointerMove={(event) => {
            ...
            >
                {columns.filter((column, idx) => idx < columns.length - 1).map((column, idx) => {
                    return (<div key={column.key} data-column-index={idx} className={`ms-card-list-column-divider${selected === idx ? ' selected' : ''}`} style={{ left: `${column.left}%` }}/>);
                })}
                {columns.map((entry) => {
                    const property = metadata.find(en => en.key === entry.key);
                    return (<div key={entry.key} className="ms-card-list-column" style={{ width: `${entry.width}%` }}>
                        <div>{property?.labelId ? <Message msgId={property.labelId}/> : null}</div>
                    </div>);
                })}
            </div>
            <div className="ms-card-list-columns-placeholder"></div>
...
const ResourcesMenu = forwardRef(({
    ...
    return (
        ...
        >
            ...
            {cardLayoutStyle === 'list' ? <CardListColumns columns={columns} setColumns={setColumns} metadata={metadata}/> : null}
        </div>
    );
});
