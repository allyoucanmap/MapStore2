...
function Facet({
    ...
    return (
        <Box className={`ms-filter-facet${active ? ' ms-selected-colors' : ''}`} pr="sm" pl={active ? 'sm' : undefined} onClick={onChange}>
            <input
                type="checkbox"
                id={filterValue}
                name={filterValue}
    ...
    if (field.type === 'filter') {
        ...
        const filterChild = () => {
            return field.items && field.items.map((item) => {
                ...
                return (
                    <Box display="flex" flexColumn flexGap="sm" key={item.uuid}>
                        {isFacet(item)
