...
function DetailsHTML({ value, placeholder }) {
    ...
    if (placeholder) {
        return (
            <Box display={expand ? undefined : "flex"} position="relative" >
                {expand
                    ? <div dangerouslySetInnerHTML={{ __html: value }} />
                    : <Box flexFill flexVerticalAlign display="flex"><Text ellipsis >{placeholder}</Text></Box>}
...
function DetailsInfoFieldEditing({ field, onChange }) {
    ...
    if (field.type === 'boolean') {
        return (
            <Box display="flex" flexGap="sm" pb="xs" row>
                <Text fontSize="sm">
...
function DetailsInfoFields({ fields, formatHref, editing, onChange, query = {} }) {
    return (<Box
        ...
        ptb="md"
        flexColumn
        className="ms-details-info-fields"
    >
