...
const getRemoveNodes = (selected) => {
    return [
        ...(selected?.id ? [ selected ] : []),
        ...(selected?.node?.nodes || []).map(getRemoveNodes).flat()
    ];
};
...
const RemoveNodesButton = connect(() => ({}), {
    ...
    function handleRemoveNodes() {
        ...
        const nodesToRemove = getRemoveNodes({ id: null, node: { nodes } });
    ...
    const isSelectedGroup = [statusTypes.GROUP, statusTypes.GROUPS].includes(status);
    const isSingleLayer = statusTypes.LAYER === status;
    return (
        <>
            {[statusTypes.LAYER, statusTypes.LAYERS, statusTypes.GROUP, statusTypes.GROUPS].includes(status) ? <ItemComponent
                {...props}
                glyph="trash"
                ...
                onClick={() => setShowDeleteDialog({ nodes: selectedNodes })}
            /> : null}
            <Portal>
                <ConfirmModal
                    options={{
