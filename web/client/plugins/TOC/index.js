...
import { NodeTypes, ROOT_GROUP_ID, DEFAULT_GROUP_ID, selectedNodesIdsToObject } from '../../utils/LayersUtils';
import { StatusTypes, isSingleDefaultGroup } from '../../utils/TOCUtils';
...
import { ControlledTOC } from '../../components/TOC/TOC';
import ContextMenu from '../../components/TOC/ContextMenu';
...
function TOC({
    ...
    const configuredItems = usePluginItems({
        items: [
            ...(activateZoomTool
                ? [
                    { name: 'TOCZoomToButton', target: 'toolbar', Component: ZoomToLayersButton, position: 4 },
                    { name: 'TOCZoomToMenuItem', target: 'context-menu', Component: ZoomToLayersButton, position: 4 }
                ]
                : []),
            { name: 'TOCReloadLayersButton', target: 'toolbar', Component: ReloadLayersButton, position: 9 },
        ...
        <div className="ms-toc-container">
            ...
            <ControlledTOC
                ...
                config={{
                    ...
                    hideOpacitySlider: !activateOpacityTool,
                    language,
                    currentLocale,
                    scales,
                    zoom,
                    showTitleTooltip: activateTitleTooltip,
                    showOpacityTooltip: !hideOpacityTooltip,
                    groupOptions,
                    layerOptions: {
                        ...layerOptions,
...
const tocSelector = createShallowSelectorCreator(isEqual)(
    ...
    (enabled, tree, layers, selectedNodes, user, map, title, currentLocale, currentLocaleLanguage, isLocalizedLayerStylesEnabled, { resolutions, resolution }, visualizationMode) => ({
        enabled,
        tree,
        layers,
