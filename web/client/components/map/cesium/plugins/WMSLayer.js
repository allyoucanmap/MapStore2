...
function wmsToCesiumOptionsSingleTile(options) {
    ...
    const parameters = assign({
        ...
        srs: "EPSG:4326"
    }, (CQL_FILTER ? {CQL_FILTER} : {}), options.params || {}, WMSUtils.getAuthenticationParam(options));
...
function wmsToCesiumOptions(options) {
    ...
    return assign({
        url: "{s}",
        subdomains: WMSUtils.getWMSURLs(isArray(options.url) ? options.url : [options.url]),
        ...
        parameters: assign({
            ...
            (options.params || {}),
            WMSUtils.getAuthenticationParam(options)
        ))
    });
}
