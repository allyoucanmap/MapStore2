...
// Map flatgeobuf header geometry type ids to the names that
// applyDefaultStyleToVectorLayer / createDefaultStyle expect.
...
const createLayer = (options, map) => {
    ...
    const applyStyle = () => {
        const geometryType = getGeometryTypeFromOptions(options) || inferredGeometryType;
    ...
    async function loadingBbox({ flatgeobuf }) {
        ...
        const reportGeometryType = (geometryType) => {
            ...
            // Config wins over inference.
            if (getGeometryTypeFromOptions(options)) {
                return;
            }
            if (inferredGeometryType === geometryType) {
                return;
            }
        ...
        const handleHeader = (headerMeta) => {
            ...
            // only use it when it maps to a concrete name.
        ...
        try {
            ...
            const iterator = flatgeobuf.deserialize(secureUrl, rect, handleHeader, false, headers);
            for await (const feature of iterator) {
                if (activeLoad !== myLoad) {
                    return;
                }
                if (!geometryReported) {
                    const fromFeature = feature?.geometry?.type;
                    if (fromFeature) {
