...
describe("test DockPanel", () => {
    ...
    it('test on select', done => {
        ...
        ReactDOM.render(<Accordion onSelect={panel => {
            expect(panel).toEqual('panel-002');
            done();
        }} activePanel="panel-001" panels={panels}/>, document.getElementById("container"));
        const domComp = document.getElementsByClassName('ms-accordion')[0];
        expect(domComp).toExist();
        const panelsDOM = document.getElementsByClassName('panel');
        expect(panelsDOM.length).toBe(2);
        const panelHead = document.getElementsByClassName('panel-heading')[1];
