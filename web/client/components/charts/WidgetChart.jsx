...
const chartDataTypes = {
    pie: ({
        ...
        const data = parsePieNoAggregationFunctionData(
            ...
            {
                parse: !hasAggregateFunction && !isNestedPieChart,
                labelDataKey,
                valueDataKey
            }
        );
        ...
        const { sorByKey, classes, classifiedData } = generateClassifiedData({
            type: 'pie',
            sortBy,
            data,
            options,
            msClassification: style?.msClassification,
            groupByValue: labelDataKey,
            legendValue: classificationDataKey,
            classifyGeoJSON
        });
        const sortedData = !isNestedPieChart && !style?.msClassification?.classes
            ...
            : classes.reduce((acc, c, idx) => {
                const filteredData = classifiedData
                    ...
                    .sort((a, b) => sorByKey === valueDataKey
                        ? a.properties[sorByKey] > b.properties[sorByKey] ? -1 : 1
                        : a.properties[sorByKey] > b.properties[sorByKey] ? 1 : -1);
    ...
    bar: ({
        ...
        if (msMode === 'classification' && classificationDataKey) {

            const { sorByKey, classifiedData, classes } = generateClassifiedData({
                type: 'bar',
                sortBy,
                data,
                options,
                msClassification,
                groupByValue: classificationDataKey,
                legendValue: yDataKey,
                classifyGeoJSON
            });
            return classes.map(({ color, label: name }, idx) => {
                const filteredData = classifiedData
                    .filter((entry) => entry.index === idx)
                    .sort((a, b) => a.properties[sorByKey] > b.properties[sorByKey] ? 1 : -1);
...
const legacyPropsToTraces = ({ series, xAxis, ...props }) => {
    if (series) {
        const options = {
