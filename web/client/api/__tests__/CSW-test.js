...
describe('Test correctness of the CSW APIs', () => {
    ...
    it('csw dct:temporal metadata YYYY-MM-DD', () => {
        let records = API.getCatalogRecords({
            records: [{
                dc: {
    ...
    it('csw with DC references, no url, no options, with uri', () => {
        ...
        const records = API.getCatalogRecords({ records: cswRecords });
        expect(records.length).toBe(1);
    ...
    it('csw with DC references, with url, with options, with uri', () => {
        ...
        const records = API.getCatalogRecords({ records: cswRecords }, {url});
        expect(records.length).toBe(1);
    ...
    it('csw with DC references, with url, with options, no uri', () => {
        ...
        const records = API.getCatalogRecords({ records: cswData }, {url});
        expect(records.length).toBe(1);
    ...
    it('csw with DC references with image mime protocol url and no thumbnail', () => {
        ...
        const records = API.getCatalogRecords({ records: cswRecordsLocal }, {url});
        expect(records.length).toBe(1);
